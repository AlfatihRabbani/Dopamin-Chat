...
const GPU_TAB_ORDER = ["none", "cuda", "optix", "rocm", "intel", "metal", "vulkan"];
const GPU_TAB_LABEL = {
  none: "None", cuda: "CUDA", optix: "OptiX", rocm: "HIP",
...
async function loadGPU() {
  let g;
  try { g = await api("/api/gpu/list"); } catch (e) { return; }
  S.gpuData = g;
...
async function send() {
  ...
  while (true) {
    ...
    for (const block of blocks) {
      ...
      if (ev === "mood") updateMood(p.dopamine);
      else if (ev === "slot") {
