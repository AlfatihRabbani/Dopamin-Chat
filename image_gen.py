...
def _comfy_list_files(url: str, kind: str) -> list[str]:
    """Query ComfyUI /object_info for available filenames of a given kind.
    kind ∈ {'CheckpointLoaderSimple','UnetLoaderGGUF','CLIPLoader','VAELoader','LoraLoader'}."""
    try:
        with urllib.request.urlopen(f"{url}/object_info/{kind}", timeout=3) as r:
            info = json.loads(r.read().decode())
        node = info.get(kind, {})
        # Each loader exposes its file list as the first element of input/required/<arg>[0].
...
def _pick_workflow(model: str, ckpts: list[str], ggufs: list[str]) -> tuple[str, str]:
    """Decide (workflow_kind, chosen_filename). Honor explicit `model` when set,
    otherwise auto-pick based on what's available. Returns ('','') if nothing usable."""
    m = (model or "").strip()
    if m:
        if m in ggufs or m.lower().endswith(".gguf"):
            return ("qwen_edit", m)
        if m in ckpts:
            low = m.lower()
            if "ltx" in low or "ltxv" in low:
                return ("ltx", m)
            return ("sdxl", m)
    # No explicit pick — auto-detect.
    sdxl = [n for n in ckpts if "ltx" not in n.lower() and "ltxv" not in n.lower()]
    if sdxl:
        return ("sdxl", sdxl[0])
    qwen = [n for n in ggufs if "qwen" in n.lower()]
    if qwen:
        return ("qwen_edit", qwen[0])
    if ggufs:
        return ("qwen_edit", ggufs[0])
    if ckpts:
        # Only LTX left — return it, caller can decide.
        return ("ltx", ckpts[0])
    return ("", "")


def _wf_sdxl(prompt, negative, width, height, steps, seed, ckpt_name):
    return {
...
def _wf_qwen_edit(prompt, negative, width, height, steps, seed,
                  unet_name, clip_name, vae_name):
    """Qwen-Image-Edit text2image via ComfyUI-GGUF loader + qwen_image CLIP type."""
    return {
        ...
        "16": {"class_type": "KSampler",
               "inputs": {"seed": int(seed), "steps": int(steps),
                          ...
                          "model": ["10", 0], "positive": ["13", 0],
                          "negative": ["14", 0], "latent_image": ["15", 0]}},
        "17": {"class_type": "VAEDecode",
               "inputs": {"samples": ["16", 0], "vae": ["12", 0]}},
        "9":  {"class_type": "SaveImage",
               "inputs": {"filename_prefix": "dopamine_qwen", "images": ["17", 0]}},
    }


def _wf_ltx(prompt, negative, width, height, steps, seed, ckpt_name):
    """LTX-Video — generates a 1-frame still (length=1) so the chat gets back
    a single image rather than a video file. For full video, use ComfyUI UI."""
    return {
        ...
        "23": {"class_type": "EmptyLTXVLatentVideo",
               "inputs": {"width": int(width), "height": int(height),
                          "length": 1, "batch_size": 1}},
        "24": {"class_type": "KSampler",
               "inputs": {"seed": int(seed), "steps": int(steps),
                          "cfg": 3.0, "sampler_name": "euler",
                          "scheduler": "normal", "denoise": 1.0,
                          "model": ["20", 0], "positive": ["21", 0],
                          "negative": ["22", 0], "latent_image": ["23", 0]}},
        "25": {"class_type": "VAEDecode",
               "inputs": {"samples": ["24", 0], "vae": ["20", 2]}},
        "9":  {"class_type": "SaveImage",
               "inputs": {"filename_prefix": "dopamine_ltx", "images": ["25", 0]}},
    }
...
def gen_comfyui(prompt: str,
    ...
    """Submit a workflow to ComfyUI based on what's installed. Auto-picks
    between SDXL, Qwen-Image-Edit GGUF, and LTX-Video workflows."""
    ...
    ckpts = _comfy_list_files(url, "CheckpointLoaderSimple")
    ggufs = _comfy_list_files(url, "UnetLoaderGGUF")
    kind, chosen = _pick_workflow(model, ckpts, ggufs)
    if not kind:
        raise RuntimeError(
            "ComfyUI has no usable models. Place a .safetensors checkpoint in "
            "ComfyUI/models/checkpoints/ or a .gguf in ComfyUI/models/unet/."
        )

    print(f"[imggen.comfy] workflow={kind} model={chosen}", flush=True)

    if kind == "qwen_edit":
        clips = _comfy_list_files(url, "CLIPLoader")
        vaes = _comfy_list_files(url, "VAELoader")
        clip = next((c for c in clips if "qwen" in c.lower()), clips[0] if clips else "")
        vae = next((v for v in vaes if "qwen" in v.lower()), vaes[0] if vaes else "")
        if not clip or not vae:
            raise RuntimeError("Qwen workflow needs qwen text_encoder + qwen_image_vae "
                               "in ComfyUI/models/text_encoders/ and /vae/.")
        # SDXL default 512 is too small for Qwen — bump if user left default.
        w = max(int(width), 1024); h = max(int(height), 1024)
        wf = _wf_qwen_edit(prompt, negative, w, h, steps, seed, chosen, clip, vae)
    elif kind == "ltx":
        wf = _wf_ltx(prompt, negative, width, height, steps, seed, chosen)
    else:
        wf = _wf_sdxl(prompt, negative, width, height, steps, seed, chosen)
    body = json.dumps({"prompt": wf, "client_id": cid}).encode("utf-8")
    req = urllib.request.Request(f"{url}/prompt", data=body,
                                 headers={"Content-Type": "application/json"})
    # Submission timeout: ComfyUI validates the workflow synchronously before
    # returning prompt_id. Cold-loading SDXL/Qwen/LTX models can block the
    # validation step for tens of seconds. 60s covers cold-load ACKs.
    submit_timeout = int(os.environ.get("DOPAMINE_COMFY_SUBMIT_TIMEOUT", "60"))
    try:
    ...
    # Poll history. Wall-clock: 10 min by default — SDXL/Qwen on 8GB VRAM with
    # CPU offload can take 5+ minutes per image. Override via env var.
    wall = int(os.environ.get("DOPAMINE_COMFY_WALL_TIMEOUT", "600"))
    deadline = time.time() + wall
    while time.time() < deadline:
        try:
            with urllib.request.urlopen(f"{url}/history/{pid}", timeout=15) as r:
        ...
        if pid in hist and "outputs" in hist[pid]:
            ...
            for node_out in outputs.values():
                for img in node_out.get("images", []):
                    ...
                    with urllib.request.urlopen(f"{url}/view?{qp}", timeout=30) as r:
                        return _save_png(r.read(), prefix="comfy")
            raise RuntimeError("comfyui finished but no images returned")
        time.sleep(1)
    raise RuntimeError(f"comfyui timeout (>{wall}s)")
