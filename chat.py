...
def gen_mode(dop: int) -> tuple[str, float, int, bool]:
    ...
    if dop > 70:  return "STABLE",  0.30, 250,  False
    return                "NEUTRAL", 0.70, 500,  False
...
def _generate_once(backend, console, p, session, sys_prompt,
    ...
    try:
        ...
        console.file.flush()
        for tok in backend.stream(messages, temp, max_new, image_pil=img):
            full += tok
            pending += tok
...
def chat_loop(console: Console, backend, session: Session):
    ...
    while True:
        ...
        if cmd == "/reset":
            ...
            console.print("[yellow]State reset: dopamine and memory cleared.[/]")
            continue
        if cmd == "/status":
