...
REM ---------- main python (chat/web) - pin to 3.11, fall back to 3.10 ----------
set "PY_CMD="
where py >nul 2>&1
if not errorlevel 1 (
    py -3.11 -V >nul 2>&1
...
REM ---------- voice python: pin to 3.11 (RVC/fairseq + numpy 2.3.x compat) ----------
set "PY311="
where py >nul 2>&1
if not errorlevel 1 (
    py -3.11 -V >nul 2>&1
    if not errorlevel 1 (
        ...
        py -3.10 -V >nul 2>&1
        if not errorlevel 1 set "PY311=py -3.10"
    )
)

REM ---------- nuke ROCm ghost env vars (broken if ROCm not actually installed) ----------
if defined HIP_PATH (
    if not exist "%HIP_PATH%\bin" (
        echo [run-windows] Clearing stale HIP_PATH ^(%HIP_PATH% missing^)
        set "HIP_PATH="
        set "HIP_PLATFORM="
        set "ROCM_PATH="
    )
...
REM Pin to 0.3.22:
REM   - 0.3.23+ wheels use AVX-512 -> illegal instruction (0xc000001d) on Zen3/older CPUs
REM   - 0.3.21 and below cannot load Gemma 3/4 GGUF format
REM cu128 index has no win_amd64 wheels yet; cu124 wheel works with cu128 torch
REM (torch loads cudart64_12.dll first; ABI compatible within CUDA 12.x).
set "LLAMA_OK=0"
set "LLAMA_VER=0.3.22"
if defined LLAMA_CUDA_INDEX (
    echo [run-windows]   trying CUDA wheel (cu124, pinned %LLAMA_VER%) ...
    pip install --only-binary=llama-cpp-python "llama-cpp-python==%LLAMA_VER%" --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu124
    if not errorlevel 1 set "LLAMA_OK=1"
)
if "!LLAMA_OK!"=="0" (
    echo [run-windows]   trying CPU wheel (pinned %LLAMA_VER%) ...
    pip install --only-binary=llama-cpp-python "llama-cpp-python==%LLAMA_VER%" --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cpu
    if not errorlevel 1 set "LLAMA_OK=1"
)
if "!LLAMA_OK!"=="0" (
    pip install --only-binary=llama-cpp-python "llama-cpp-python==%LLAMA_VER%"
    if not errorlevel 1 set "LLAMA_OK=1"
)
if "!LLAMA_OK!"=="0" (
