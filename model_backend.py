...
# ---------------------------------------------------------------------------
# Stop strings — halt generation if the model starts hallucinating a new
# user turn. Kept here so both backends share the same list.
# ---------------------------------------------------------------------------
STOP_STRINGS = [
    "\nUSER:", "\nUser:", "\nuser:",
    "USER:", "User:",
    ...
    def stream(self, messages, temp, max_new, image_pil=None, seed=None):
        ...
        class _StopOnStrings(StoppingCriteria):
            ...
            def __call__(self, input_ids, scores, **kw):
                new = input_ids[0, start_len:]
                text = tok_ref.decode(new, skip_special_tokens=True)
        ...
        kwargs = dict(
            **inputs, streamer=streamer, do_sample=True,
            ...
            stopping_criteria=StoppingCriteriaList([_StopOnStrings(STOP_STRINGS)]),
        )
        ...
        try:
            ...
            for tok in streamer:
                buf += tok
                # Client-side cut: trim everything from the first stop substring.
                cut = -1
                ...
                if cut >= 0:
                    safe_now = buf[:cut]
                    emitted = buf[:-len(tok)] if tok else buf
                    extra = safe_now[len(emitted):] if len(safe_now) > len(emitted) else ""
                    if extra:
                        yield extra
                    break
                yield tok
...
class GGUFBackend(Backend):
    ...
    def __init__(self, gguf_path: Path, console: Console):
        ...
        if chat_handler is not None:
            ...
            # NOTE: do NOT set logits_all=True. Vision handler doesn't need it,
            # and it forces logits computation for every prompt token instead
            # of just the last → 5-10× slower generation.
        ...
        try:
            ...
            try:
                ...
                cpu_kwargs["use_mmap"] = True   # mmap is ideal for CPU path
